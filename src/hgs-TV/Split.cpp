...
namespace genvrp {
    ...
    void Split::generalSplit(Individual* indiv, int nbMaxVehicles) {
        ...
        maxVehicles = std::max<int>(nbMaxVehicles, std::ceil(params.data.totalDemand / params.data.vehicleCapacity));

        // Initialization of the data structures for the linear split algorithms
        for(int i = 1; i <= params.data.nbClients; i++) {
            cliSplit[i].demand = params.data.cli[indiv->giantTour[i - 1]].demand;
            cliSplit[i].serviceTime = params.data.cli[indiv->giantTour[i - 1]].serviceDuration;
            cliSplit[i].d0_x = params.data.timeCost[0][indiv->giantTour[i - 1]];
            cliSplit[i].dx_0 = params.data.timeCost[indiv->giantTour[i - 1]][0];
            if(i < params.data.nbClients)
                cliSplit[i].dnext = params.data.timeCost[indiv->giantTour[i - 1]][indiv->giantTour[i]];
            else
    ...
    int Split::splitSimple(Individual* indiv) {
        ...
        // This code has been maintained as it is very simple and can be easily adapted to a variety of constraints, whereas the O(n) Split has a more
        // restricted application scope
        if(params.data.isDurationConstraint) {
            for(int i = 0; i < params.data.nbClients; i++) {
                double load = 0.;
                double distance = 0.;
                double serviceDuration = 0.;
                for(int j = i + 1; j <= params.data.nbClients && load <= 1.5 * params.data.vehicleCapacity; j++) {
                    ...
                    double cost = distance + cliSplit[j].dx_0 + params.ga.penaltyCapacity * std::max<double>(load - params.data.vehicleCapacity, 0.) +
                                  params.ga.penaltyDuration * std::max<double>(distance + cliSplit[j].dx_0 + serviceDuration - params.data.durationLimit, 0.);
            ...
            Trivial_Deque queue = Trivial_Deque(params.data.nbClients + 1, 0);
            for(int i = 1; i <= params.data.nbClients; i++) {
        ...
        if(potential[0][params.data.nbClients] > 1.e29)
            throw std::string("ERROR : no Split solution has been propagated until the last node");

        // Filling the routes structure
        for(int k = params.data.nbVehicles - 1; k >= maxVehicles; k--)
            indiv->routes[k].clear();

        int end = params.data.nbClients;
        for(int k = maxVehicles - 1; k >= 0; k--) {
            indiv->routes[k].clear();
            int begin = pred[0][end];
            for(int ii = begin; ii < end; ii++)
                indiv->routes[k].push_back(indiv->giantTour[ii]);
            end = begin;
        }
    ...
    int Split::splitLF(Individual* indiv) {
        ...
        if(params.data.isDurationConstraint) {
            for(int k = 0; k < maxVehicles; k++) {
                for(int i = k; i < params.data.nbClients && potential[k][i] < 1.e29; i++) {
                    double load = 0.;
                    double serviceDuration = 0.;
                    double distance = 0.;
                    for(int j = i + 1; j <= params.data.nbClients && load <= 1.5 * params.data.vehicleCapacity;
                        j++) // Setting a maximum limit on load infeasibility to accelerate the algorithm
                    {
                        load += cliSplit[j].demand;
                        serviceDuration += cliSplit[j].serviceTime;
                        if(j == i + 1)
                            ...
                            distance += cliSplit[j - 1].dnext;
                        double cost =
                            distance + cliSplit[j].dx_0 + params.ga.penaltyCapacity * std::max<double>(load - params.data.vehicleCapacity, 0.) +
                            params.ga.penaltyDuration * std::max<double>(distance + cliSplit[j].dx_0 + serviceDuration - params.data.durationLimit, 0.);
        ...
        } else // MAIN ALGORITHM -- Without duration constraints in O(n), from "Vidal, T. (2016). Split algorithm in O(n) for the capacitated vehicle routing
               // problem. C&OR"
        {
            Trivial_Deque queue = Trivial_Deque(params.data.nbClients + 1, 0);
            for(int k = 0; k < maxVehicles; k++) {
                ...
                for(int i = k + 1; i <= params.data.nbClients && queue.size() > 0; i++) {
                    // The front is the best predecessor for i
        ...
        if(potential[maxVehicles][params.data.nbClients] > 1.e29)
            throw std::string("ERROR : no Split solution has been propagated until the last node");

        // It could be cheaper to use a smaller number of vehicles
        double minCost = potential[maxVehicles][params.data.nbClients];
        int nbRoutes = maxVehicles;
        for(int k = 1; k < maxVehicles; k++)
            if(potential[k][params.data.nbClients] < minCost) {
                minCost = potential[k][params.data.nbClients];
                nbRoutes = k;
            }

        // Filling the routes structure
        for(int k = params.data.nbVehicles - 1; k >= nbRoutes; k--)
            indiv->routes[k].clear();
