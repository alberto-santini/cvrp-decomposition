...
namespace genvrp {
    ...
    struct Node {
        ...
        int cour;                         // Node index
            int whenLastTestedRI;                    // Last time the moves associated to this node have been tested without any improvement
    ...
    private:
        ...
        int nbMoves; // Total number of moves (RI and SWAP*) applied during the local search. Attention: this is not only a simple counter, it is also used to
                     // avoid repeating move evaluations
        ...
        void setLocalVariablesRouteU(); // Initializes some local variables and distances associated to routeU to avoid always querying the same values in the
                                        // distance matrix
        void setLocalVariablesRouteV(); // Initializes some local variables and distances associated to routeV to avoid always querying the same values in the
                                        // distance matrix
...
#define penaltyExcessDuration(x)                                                                                                                               \
    0. // <--- Use this line instead of the previous one to save some CPU time if your problem does not include duration constraints
        inline double _penaltyExcessDuration(double myDuration) { return std::max<double>(0., myDuration - params.data.durationLimit) * penaltyDurationLS; }
        inline double penaltyExcessLoad(double myLoad) { return std::max<double>(0., myLoad - params.data.vehicleCapacity) * penaltyCapacityLS; }
        ...
        double getCheapestInsertSimultRemoval(Node* U, Node* V,
